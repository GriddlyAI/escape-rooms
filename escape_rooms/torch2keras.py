...
from models import BipedalWalkerStudentPolicy
from util import make_agent
...
def load_model(xp_path, model_tar):
	model = None
	...
	if xp_flags.env_name.startswith('BipedalWalker'):
		env = gym_make(xp_flags.env_name)			
		agent = make_agent(name='agent', env=env, args=xp_flags)
		model = agent.algo.actor_critic
	else:
		raise ValueError(f'Unsupported environment {xp_flags.env_name}.')

	model_checkpoint = torch.load(model_path, map_location='cpu')
	model.load_state_dict(model_checkpoint['runner_state_dict']['agent_state_dict']['agent'])

	return model, xp_flags


def convert2tf_and_export(model, xp_flags, export_path):
	if xp_flags.env_name.startswith('BipedalWalker'):
		x = torch.randn(1, 24, requires_grad=False) # dummy input
		tf_model = pytorch_to_tf(model, x, [(24,)], verbose=True, export_path=export_path)
		return tf_model
	else:
		raise ValueError(f'Unsupported environment {xp_flags.env_name}.')
...
if __name__ == '__main__':
	...
	for i, xpid in enumerate(xpids):
		...
		model, xp_flags = load_model(xp_path, args.model_tar)
